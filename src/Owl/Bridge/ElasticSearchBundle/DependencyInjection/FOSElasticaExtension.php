...
class FOSElasticaExtension extends Extension
{
    ...
    private function loadClients(array $clients, ContainerBuilder $container): void
    {
        foreach ($clients as $name => $clientConfig) {
            $clientBuilderId = \sprintf('fos_elastica.client_builder.%s', $name);
            $clientId = \sprintf('fos_elastica.client.%s', $name);

            $this->loadClientBuilder($clientBuilderId, $clientConfig, $container);

            $clientDef = new ChildDefinition('fos_elastica.client_prototype');
            $clientDef
                ->setFactory([new Reference($clientBuilderId), 'build']);
    ...
    private function loadClientBuilder(string $clientBuilderId, array $clientConfig, ContainerBuilder $container): void
    {
        $connectionConfig = $clientConfig['connection'];
        $clientBuilderDef = new ChildDefinition('fos_elastica.client_builder_prototype');

        $logger = $connectionConfig['logger'];
        if (false !== $logger) {
            $clientBuilderDef->addMethodCall('setLogger', [new Reference($logger)]);
        }

        $clientBuilderDef->addMethodCall('setHosts', [$connectionConfig['hosts']]);
        $clientBuilderDef->addMethodCall('setLogger', [new Reference('logger')]);
        $clientBuilderDef->addMethodCall('setSSLVerification', [$connectionConfig['ssl_verification']]);
        $clientBuilderDef->addMethodCall('setCABundle', [$connectionConfig['ssl_ca']]);

        if(!empty($connectionConfig['username']) && !empty($connectionConfig['password'])) {
            $clientBuilderDef->addMethodCall('setBasicAuthentication', [$connectionConfig['username'], $connectionConfig['password']]);
        }

        $container->setDefinition($clientBuilderId, $clientBuilderDef); 
    }
    ...
    private function loadIndexes(array $indexes, ContainerBuilder $container): void
    {
        ...
        foreach ($indexes as $name => $index) {
            ...
            $indexDef->replaceArgument(0, new Reference('fos_elastica.client'));
            $indexDef->replaceArgument(1, $indexName);
            ...
            if (isset($index['client'])) {
                ...
                $indexDef->replaceArgument(0, $client);
            }

            $container->setDefinition($indexId, $indexDef);
            $reference = new Reference($indexId);

            $registry = $container->getDefinition('fos_elastica.registry.index');
            $registry->addMethodCall('register', [$indexName, $reference]);
    ...
    private function loadIndexFinder(ContainerBuilder $container, string $name, Reference $index): void
    {
        ...
        $registry = $container->getDefinition('fos_elastica.registry.finder');
        $registry->addMethodCall('register', [$name, new Reference($finderId)]); 
    }
    ...
    private function loadTypeFinder(array $typeConfig, ContainerBuilder $container, string $elasticaToModelId, Reference $indexRef, string $indexName): string
    {
        if (isset($typeConfig['finder']['service'])) {
            ...
            $registry = $container->getDefinition('fos_elastica.registry.finder');
            $registry->addMethodCall('register', [$indexName, new Reference($finderId)]); 
        }
