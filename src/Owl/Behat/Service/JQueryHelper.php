...
abstract class JQueryHelper
{
    ...
    {
        $document->waitFor($timeout, function () use ($document) {
            return !$document->find('css', 'form.loading');
        });
    }

    public static function waitForModalToStopLoading(DocumentElement $document, int $timeout = 10): void
    {
        $document->waitFor($timeout, function () use ($document) {
            return !$document->find('css', 'div.loader');
        });
    }
}
