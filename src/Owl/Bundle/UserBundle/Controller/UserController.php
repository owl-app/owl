...
use Owl\Bundle\UserBundle\Form\Type\UserChangePasswordType;
use Owl\Bundle\UserBundle\Form\Type\UserRequestPasswordResetType;
use Owl\Bundle\UserBundle\Form\Type\UserResetPasswordType;
...
class UserController extends BaseController
{
    ...
    public function requestPasswordResetTokenAction(Request $request): Response
    {
        ...
        $generator = $this->container->get(sprintf('owl.%s.token_generator.password_reset', $this->metadata->getName()));
    ...
    public function resetPasswordAction(Request $request, string $token): Response
    {
        ...
        $isValid = in_array($request->getMethod(), ['POST', 'PUT', 'PATCH'], true) ? $form->handleRequest($request)->isValid() : false;

        if (in_array($request->getMethod(), ['POST', 'PUT', 'PATCH'], true) && $configuration->isAjaxRequest() && !$isValid) {
            try {
    ...
    public function verifyAction(Request $request, string $token): Response
    {
        ...
        if (null === $user) {
            ...
            $this->addTranslatedFlash('error', 'owl.user.verify_email_by_invalid_token');
        ...
        $flashMessage = $this->getSyliusAttribute($request, 'flash', 'owl.user.verify_email');
    ...
    public function requestVerificationTokenAction(Request $request): Response
    {
        ...
        if (null === $user) {
            ...
            $this->addTranslatedFlash('notice', 'owl.user.verify_no_user');
        ...
        if (null !== $user->getVerifiedAt()) {
            ...
            $this->addTranslatedFlash('notice', 'owl.user.verify_verified_email');

            return $this->redirectHandler->redirectToRoute($configuration, $redirectRoute);
        }

        /** @var GeneratorInterface $tokenGenerator */
        $tokenGenerator = $this->container->get(sprintf('owl.%s.token_generator.email_verification', $this->metadata->getName()));
        ...
        $this->addTranslatedFlash('success', 'owl.user.verify_email_request');
    ...
    protected function prepareResetPasswordRequest(Request $request, GeneratorInterface $generator, string $senderEvent): Response
    {
        ...
        if (in_array($request->getMethod(), ['POST', 'PUT', 'PATCH'], true) && $isValid) {
            ...
            $this->addTranslatedFlash('success', 'owl.user.reset_password_request');
            $redirectRoute = $this->getSyliusAttribute($request, 'redirect', null);
    ...
    protected function handleResetPassword(
        ...
        $this->manager->flush();
        $this->addTranslatedFlash('success', 'owl.user.reset_password');
