...
namespace Owl\Bundle\UiBundle\DependencyInjection\Compiler;
...
final class LiveComponentTagPass implements CompilerPassInterface
{
    ...
    {
        $liveComponentTags = $container->getParameter('owl_ui.twig_ux.live_component_tags') ?? [];
        $defaultTemplate = $container->getParameter('owl_ui.twig_ux.component_default_template');

        foreach ($liveComponentTags as $name => $tagOptions) {
            foreach ($container->findTaggedServiceIds(sprintf('owl.live_component.%s', $name)) as $id => $tags) {
                foreach ($tags as $tag) {
                    $liveComponentService = $container->getDefinition($id);
                    $liveComponentService->addTag('twig.component', [
                        'key' => $tag['key'] ?? $tagOptions['key'] ?? throw new InvalidArgumentException('The "key" attribute is required for the "owl.live_component" tag'),
