...
final class LiveComponentTagPassTest extends AbstractCompilerPassTestCase
{
    ...
    {
        parent::setUp();
        $this->container->setParameter('owl_ui.twig_ux.live_component_tags', []);
        $this->container->setParameter('owl_ui.twig_ux.component_default_template', '@OwlUi/components/default.html.twig');
    }

    public function testAddingTwigComponentTagToServicesTaggedWithLiveComponentTag(): void
    {
        $liveComponent = new Definition();
        $liveComponent->addTag('owl.live_component.ui', ['key' => 'foo', 'template' => 'bar']);

        $this->setParameter('owl_ui.twig_ux.live_component_tags', ['ui' => ['route' => 'owl_ui_live_component']]);
    ...
    {
        $liveComponent = new Definition();
        $liveComponent->addTag('owl.live_component.ui', [
            'key' => 'foo',
            'template' => 'bar',
    ...
    public function testOverridingTagAttributesWithDefaultTagOptions(): void
    {
        ...
        $this->setParameter(
            'owl_ui.twig_ux.live_component_tags',
            [
                'ui' => [
                        'expose_public_props' => false,
                        'attributes_var' => 'custom_attributes',
    ...
    {
        $liveComponent = new Definition();
        $liveComponent->addTag('owl.live_component.ui', ['template' => 'bar']);

        $this->setParameter('owl_ui.twig_ux.live_component_tags', ['ui' => ['route' => 'owl_ui_live_component']]);
        $this->setDefinition('my_live_component', $liveComponent);

        $this->expectException(\InvalidArgumentException::class);
        $this->expectExceptionMessage('The "key" attribute is required for the "owl.live_component" tag');
